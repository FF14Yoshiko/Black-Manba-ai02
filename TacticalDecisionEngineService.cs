...
public sealed class TacticalDecisionEngineService
{
        ...
        AnnouncementDecisionContext announcementContext)
    {
        ...
        if (mapTactics.IsAvailable)
        {
            ...
            mapRisk = Math.Clamp(topZoneRisk * 0.50f + routeRisk * 0.25f + heatRisk * 0.25f, 0f, 100f);
        }
        ...
        objectiveRisk = Math.Clamp(objectiveRisk + chatObjectiveRisk.ObjectiveRiskBonus, 0f, 100f);
        combatRisk = Math.Clamp(combatRisk + chatObjectiveRisk.CombatRiskBonus, 0f, 100f);

        var limitBreakRisk = Math.Clamp(teamSituation.LimitBreakThreats.EnemyHighThreatCount * 24f + teamSituation.LimitBreakThreats.EnemyLikelyReadyCount * 12f, 0f, 100f);
        ...
        objectiveRisk = Math.Clamp(objectiveRisk + announcementRisk.ObjectiveRiskBonus, 0f, 100f);
        limitBreakRisk = Math.Clamp(limitBreakRisk + announcementRisk.LimitBreakRiskBonus, 0f, 100f);
        ...
        var terrainRisk = Math.Clamp(ResolveTerrainRisk(mapTactics) + announcementRisk.TerrainRiskBonus + snowBlessingRisk.TerrainRiskBonus, 0f, 100f);
        ...
        var overall = Math.Clamp(
            weightedOverallRisk / OverallRiskWeightTotal,
            0f,
            100f);

        var level = overall >= 78f ? "极高" : overall >= 62f ? "高" : overall >= 42f ? "中" : "低";
        mapRisk = Math.Clamp(terrainRisk * 0.35f + retreatRouteRisk * 0.35f + flankRisk * 0.30f, 0f, 100f);
        var announcementText = string.IsNullOrWhiteSpace(announcementRisk.EvidenceText)
            ? string.Empty
            : $"，通告 {announcementRisk.EvidenceText}";
        var summary = $"总体风险 {level} {overall:0}：夹击 {flankRisk:0}，人数差 {numberDisadvantageRisk:0}，复活 {respawnRisk:0}，敌方方向 {enemyDirectionRisk:0}，地形 {terrainRisk:0}，出路 {retreatRouteRisk:0}，被包 {encirclementRisk:0}，追击陷阱 {ambushRisk:0}，跟随散乱 {cohesionRisk:0}，第三方 {thirdPartyPincerRisk:0}，封路 {chokeBlockRisk:0}，技能 {skillThreatRisk:0}，帧事件 {frameEventRisk:0}{announcementText}";
        if (!string.IsNullOrWhiteSpace(chatObjectiveRisk.EvidenceText))
            summary += $"；目标事件 {chatObjectiveRisk.EvidenceText}";
        if (!string.IsNullOrWhiteSpace(snowBlessingRisk.EvidenceText))
            summary += $"；雪精祝福 {snowBlessingRisk.EvidenceText}";
    ...
    private static ChatObjectiveRiskModifier ResolveChatObjectiveRiskModifier(BattlefieldChatEventSituationSnapshot chatEvents)
    {
        ...
        if (chatEvents.LatestObjectiveEvent.HasValue && chatEvents.LatestObjectiveEvent.Value.AgeMs <= 30000)
        {
            ...
            var targetName = BuildChatObjectiveTargetName(item, FrontlineMapType.Unknown);
            switch (item.Kind)
            {
                ...
                case BattlefieldChatEventKind.ObjectiveLost:
                    ...
                    flankRisk += 4f;
                    parts.Add(item.ActorSide == BattlefieldTacticalSide.Friendly ? $"{targetName}刚丢" : $"{targetName}失去归属");
                    break;
                case BattlefieldChatEventKind.ObjectiveCaptured:
                    ...
                    parts.Add(item.ActorSide == BattlefieldTacticalSide.Friendly ? $"{targetName}已拿" : $"{targetName}敌方已拿");
                    break;
                case BattlefieldChatEventKind.ObjectiveOther:
                    objectiveRisk += 4f;
                    parts.Add($"{targetName}变化");
                    break;
            }
        }

        if (chatEvents.ObjectiveEventsRecent >= 2)
            parts.Add($"近30秒{chatEvents.ObjectiveEventsRecent}次目标事件");
        ...
        BattlefieldChatEventSituationSnapshot chatEvents)
    {
        ...
        if (mapTactics.IsAvailable)
        {
            var flankZoneRisk = mapTactics.TopZones
                .Where(zone => zone.Kind == MapAnnotationKind.Flank || zone.Label.Contains("夹", StringComparison.OrdinalIgnoreCase))
                .Select(zone => zone.TotalRisk)
                .DefaultIfEmpty(0f)
                .Max();
            var flankHeatRisk = mapTactics.HeatPoints
                .Where(point => point.SourceText.Contains("夹", StringComparison.OrdinalIgnoreCase))
    ...
    private static float ResolveTerrainRisk(BattlefieldMapTacticsSnapshot mapTactics)
    {
        ...
        var badZoneRisk = mapTactics.TopZones
            .Where(zone => zone.Kind is MapAnnotationKind.Danger or MapAnnotationKind.LowGround or MapAnnotationKind.Underpass or MapAnnotationKind.Flank
                ...
                || zone.Recommendation.Contains("绕", StringComparison.Ordinal))
            .Select(zone => zone.TotalRisk)
            .DefaultIfEmpty(0f)
            .Max();
        ...
        var highGroundRelief = mapTactics.TopZones
            .Where(zone => zone.Kind == MapAnnotationKind.HighGround && zone.Recommendation.Contains("防守", StringComparison.Ordinal))
            .Select(zone => 12f)
            .DefaultIfEmpty(0f)
            .Max();

        return Math.Clamp(badZoneRisk * 0.48f + routeRisk * 0.28f + heatRisk * 0.24f - highGroundRelief, 0f, 100f);
    }
    ...
    private static bool IsRetreatRoute(BattlefieldMapTacticalRouteSnapshot route)
    {
        ...
        return text.Contains("撤", StringComparison.OrdinalIgnoreCase)
            || text.Contains("退", StringComparison.OrdinalIgnoreCase)
            || text.Contains("回", StringComparison.OrdinalIgnoreCase)
            || text.Contains("返回", StringComparison.OrdinalIgnoreCase)
            || text.Contains("出生", StringComparison.OrdinalIgnoreCase)
            || text.Contains("安全", StringComparison.OrdinalIgnoreCase)
            || text.Contains("retreat", StringComparison.OrdinalIgnoreCase)
        ...
        int etaSeconds)
    {
        ...
        if (mapTactics.IsAvailable)
        {
            ...
            var routeRisk = mapTactics.Routes
                ...
                .Select(route => route.TotalRisk * 0.18f)
                .DefaultIfEmpty(0f)
                .Max();
            risk += zoneRisk * 0.20f + heatRisk * 0.18f + routeRisk;
        }
        ...
        BattlefieldPlayerFrameEventSituationSnapshot playerFrameEvents)
    {
        ...
        var mapTacticsReliability = !mapTactics.IsAvailable
            ...
            : Math.Clamp(
                ...
                + Math.Min(12f, mapTactics.Routes.Length * 1.5f)
        ...
        Vector3 anchor)
    {
        ...
        candidates.Add(CreateActionCandidate(
            ...
            teamSituation.IsEnemySplit ? "敌方分兵，侧翼有窗口" : "地图路径允许主动侧压",
            $"敌方分兵 {teamSituation.IsEnemySplit}；总体 {risk.OverallRisk:0}；路径 {(route.HasValue ? route.Value.TotalRisk.ToString("0") : "无")}",
        ...
        if (route.HasValue && route.Value.TotalRisk <= 78f && risk.RetreatRouteRisk <= 82f)
        {
            candidates.Add(CreateActionCandidate(
                ...
                "侧翼路径风险可接受，可以压敌方后撤方向",
                $"路径 {route.Value.RouteId}；路径风险 {route.Value.TotalRisk:0}；出路风险 {risk.RetreatRouteRisk:0}",
    ...
    private static string BuildRouteText(BattlefieldMapTacticalRouteSnapshot? route, BattlefieldActionType actionType)
    {
        if (!route.HasValue)
        {
            return actionType switch
            {
                ...
                BattlefieldActionType.Flank => "沿侧翼路径推进，和主团同步夹击",
                BattlefieldActionType.WrapBehind or BattlefieldActionType.BacklinePressure => "沿侧后路径推进，压敌方退路但保留回接点",
                BattlefieldActionType.Rotate or BattlefieldActionType.TouchObjective or BattlefieldActionType.AttackIce => "按当前最快路径前往",
                _ => "无需固定路径",
            };
        }
        ...
        var label = !string.IsNullOrWhiteSpace(value.Recommendation)
            ...
            : !string.IsNullOrWhiteSpace(value.KindSummary) ? value.KindSummary : "地图路径";
